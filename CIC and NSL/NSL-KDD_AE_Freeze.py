...
class Autoencoder_Test():
    ...
    def main(self):
        ...
        autoencoder_train = self.autoencoder_model.fit(train_X, train_X, epochs=self.epochs,
                                                       batch_size=self.batch_size, validation_split=0.2, callbacks=[cb_checkpoint])
        ...
        import time
        start = time.time()
        unFreeze_classify_train = self.full_model.fit(train_X, Onehot_train_Y2, batch_size=self.batch_size, epochs=self.epochs, verbose=1, validation_split=0.2)
        end = time.time()
